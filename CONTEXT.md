...
`harness/day6_scaling_harness.py`, built this session and not yet run. 23 August (Day 5):
a follow-up n=18 warm-cache latency measurement (5 additional live `/api/analyze_apk`
calls per demo APK, on top of the demo-capture sample) found the kernel-memory-pressure
onset documented in D-9/Finding 6 much earlier than previously measured — within ~4 calls
here, not 15+ as in the Day 6 batch. §6's D-9 row updated with a pointer; full data in
`SESSION_LOG.md`. A fresh-boot, three-call, stage-order measurement followed
(14.66s/6.7s/6.42s), confirming the live demo is viable under fresh-boot conditions;
`demo/DEMO_RUNBOOK.md` was updated with the timing-claim removal and a reboot pre-flight
step.
...
| D-9 | ~~Endpoint timings have no producing harness (§7 U1).~~ **PARTIALLY RESOLVED 22 Aug (Day 6).** `harness/day6_scaling_harness.py`'s `latency` mode was run against the live `/api/analyze_apk` endpoint, `harness/banking_legit_corpus/`, files <=50 MB: wall-clock median 123.9s, IQR 107.1-145.7s, **n=14 valid of 30 requested/sampled** (16 voided — kernel paged Mistral-7B out partway through under memory pressure; size-distribution check ruled out a file-size artifact; the 14 valid runs are the first 14 in execution order). Full results: `harness/DAY6_SCALING_RESULTS.json`. `size-sweep` and `demo-footprint` modes are **OUTSTANDING** — not yet run; need a clean machine with all dev tooling closed. **23 Aug (Day 5) addendum, negative result:** a separate n=18 warm-cache measurement (three demo APKs, 6 calls each) found the same kernel-memory-pressure mechanism onsetting within ~4 calls, not 15+ as in the Day 6 batch — the threshold does not generalize across corpora/batch composition. Root cause reconfirmed directly (`ollama ps` showed `size_vram: 0` on both models while `expires_at` still held the `keep_alive=-1` sentinel — Ollama's bookkeeping believes the models are resident, the kernel disagrees). Full data in `SESSION_LOG.md`, 2026-08-23 (Day 5) entry. Verdict/risk_score/verdict_source held constant across all 18 calls — correctness surface unaffected. A follow-up fresh-boot, three-call, stage-order measurement (14.66s/6.7s/6.42s) found the live demo viable under fresh-boot conditions; `demo/DEMO_RUNBOOK.md` was updated with the timing-claim removal and a reboot pre-flight step — full reboot, not a Flask restart, required, since a no-reboot re-capture measured 3.63s/6.54s/96.62s over the same three-call sequence, confirming a fresh Flask process is not equivalent to a fresh machine. | — | **Medium-High.** Latency now measured from three independent angles; onset timing does not generalize across corpora, but the live-demo-viability question is answered — viable under fresh-boot conditions, per the updated runbook. | 18 Aug, `PLAN.md` 8 |
